...
const baseSchema = {
    questionType: Yup.string().trim()
        .required("Select a question type").oneOf(["objective", "germane"])
        .label("Question Type"),


    questionContent: Yup.string().trim().required("Fill in the question text").label("Question Content").min(64, "Too small!").max(512, "Too long, rephrase it"),
...
const objQuestionSchemaExtension = {
    correctOption: Yup.string().trim().oneOf(['a', 'b', 'c', 'd']).required("Select the correct option").label("Correct Option"),

    optionA: Yup.string().trim().min(1, "Too small!").max(512, "Too long, rephrase it").required("Fill in the option").label("Option A"),

    optionB: Yup.string().trim().min(1, "Too small!").max(512, "Too long, rephrase it").required("Fill in the option").label("Option B"),
    optionC: Yup.string().trim().min(1, "Too small!").max(512, "Too long, rephrase it").required("Fill in the option").label("Option C"),
    optionD: Yup.string().trim().min(1, "Too small!").max(512, "Too long, rephrase it").required("Fill in the option").label("Option D"),
}


const germaneQuestionSchemaExtension = {
    answer: Yup.string().trim().min(1, "Too small!").max(512, "Too long, rephrase it").required("Fill in the answer").label("Answer"),
}
...
export default function AddQuestionToCourseForm({ close, course }) {
    ...
    const yupSchema = Yup.object().shape(
        ...
        schemaCombination === 0 ? baseSchema : schemaCombination === 1 ? { ...baseSchema, ...objQuestionSchemaExtension } : schemaCombination === 2 ? { ...baseSchema, ...germaneQuestionSchemaExtension } : baseSchema
                    ...
                    return (
                        <Form className="bg-gray-30   p-4 w-[80%] mx-auto ">
                            ...
                            <div>
                                ...
                                <CustomSelect defaultValue={{ name: values.questionType, value: values.questionType }} showNullOption={false} auth={store.auth} currentValue={values.questionType} name="questionType" error={errors.questionType} setValue={(v) => {
                                ...
                                }} params={{ data: questionTypes, nameKey: "name", valueKey: "value" }} />
...

                            {
                                ...
                                <div>
                                    ...
                                    <CustomRadios radios={correctOptionRadios} label="Correct Option" name="correctOption" type="radio" required />
                                ...
                                {!readerReady && (
                                    <>
                                        ...
                                        >
                                            ...
                                            <span className="text-xs block pt-1 font-normal text-blue-500"> Only include an illustration if the question references it </span>
                                        </label>
